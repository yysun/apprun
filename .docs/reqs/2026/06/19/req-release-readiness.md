# 6.0.0 Release Readiness Requirements
...
Release readiness must turn the completed roadmap implementation work into a coherent release candidate. The package version must move to `6.0.0`, the runtime version constant must match it, CI must verify test/build/lint/pack gates, and release-facing docs must describe the breaking API, package output policy, and optional `immer` dependency clearly.
...
- [x] RPD docs record release-readiness scope, validation, commit boundaries, and final evidence.
...
- Do not rewrite the already committed roadmap behavior work.
...
- Release-readiness commit boundaries:
  - `5eba13f` RPD requirements and plan
  - `d10be72 ci: verify lint and package dry run`
  - `1203e12 chore: release 6.0.0`
  - `a794fc6` final RPD evidence
