...
const app = _app;
export default app;
const shouldRouteLinkClick = (e, menu) => {
    if (!menu)
        return false;
    ...
    if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey)
        return false;
    if (menu.target && menu.target.toLowerCase() !== '_self')
        return false;
    if (menu.hasAttribute('download'))
        return false;
    if ((menu.getAttribute('rel') || '').toLowerCase().split(/\s+/).includes('external'))
        return false;
    return menu.origin === location.origin && !!menu.pathname;
};
const routeLinkClick = (e, menu) => {
    if (!shouldRouteLinkClick(e, menu))
        return false;
    e.preventDefault();
